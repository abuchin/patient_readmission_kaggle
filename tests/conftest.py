...
def sample_diabetes_data(test_data_dir):
    ...
    data = {
        'race': np.random.choice(['Caucasian', 'AfricanAmerican', 'Hispanic', 'Other'], n_samples),
        'gender': np.random.choice(['Male', 'Female'], n_samples),
        'age': np.random.choice(['[70-80)', '[60-70)', '[50-60)', '[40-50)', '[30-40)'], n_samples),
...
def monitoring_data(test_data_dir):
    ...
    cur_data = pd.DataFrame({
        f'feature_{i}': np.random.randn(n_samples) + (0.1 if i < 5 else 0) for i in range(20)
    })
