...
def prepare_data_for_prediction(df, fit_preprocessor=None):
    ...
    if fit_preprocessor is None:
        ...
        print(
            f"🔧 Created preprocessor with {len(numeric_cols)} numeric and {len(categorical_cols)} categorical features"
        )
    else:
    ...
    try:
        ...
        print(
            f"✅ Preprocessed data: {X_processed.shape[0]} rows × {X_processed.shape[1]} features"
        )
...
def score_with_preprocessing(df, endpoint, tmp_csv_path, pred_col="pred_readmit"):
    ...
    try:
        ...
        if len(predictions) != len(df):
            raise ValueError(
                f"Prediction count {len(predictions)} != data rows {len(df)}"
            )
...
def main():
    ...
    ap = argparse.ArgumentParser(
        description="Real monitoring script with preprocessing"
    )
