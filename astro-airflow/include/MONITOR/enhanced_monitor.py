...
def create_training_compatible_preprocessor(reference_df):
    ...
    print(
        f"🔧 Creating preprocessor from reference data: {len(feature_df)} rows, {len(feature_df.columns)} cols"
    )
...
def preprocess_for_ml_service(df, preprocessor, feature_names):
    ...
    try:
        ...
        print(
            f"   ✅ Transformed: {X_processed.shape[0]} rows × {X_processed.shape[1]} features"
        )
...
def score_with_consistent_preprocessing(
    ref_df, cur_df, endpoint, tmp_dir, pred_col="pred_readmit"
):
    """
    ...
    preprocessor, feature_names, numeric_cols, categorical_cols = (
        create_training_compatible_preprocessor(ref_df)
    )
...
def main():
    ...
    ap = argparse.ArgumentParser(
        description="Enhanced monitoring script with consistent preprocessing"
    )
    ...
    if summary.get("feature_results"):
        ...
        print(
            f"   📊 Features with drift: {drift_count}/{len(summary['feature_results'])}"
        )

        # Show top 5 features by PSI
        sorted_features = sorted(
            summary["feature_results"], key=lambda x: x["psi"], reverse=True
        )[:5]
