...
def post_csv(endpoint: str, csv_path: str, timeout: int = 60):
    with open(csv_path, "rb") as f:
        r = requests.post(
            endpoint, headers={"Content-Type": "text/csv"}, data=f, timeout=timeout
        )
    r.raise_for_status()
    try:
        return r.json()
...
def _coerce_to_float_list(
    ...
    if isinstance(raw, str):
        ...
        try:
            ...
            raise RuntimeError(
                f"Server returned non-JSON text (first 200 chars): {raw[:200]}"
            ) from e
    ...
    if isinstance(raw, list) and len(raw) > 0 and isinstance(raw[0], dict):
        ...
        if key is None:
            raise ValueError(
                "Predictions are list of dicts; pass --pred-key (e.g. --pred-key pred_proba)."
            )
...
def score_and_attach(
    ...
    if len(preds) != len(df):
        raise ValueError(
            f"Prediction length {len(preds)} != number of rows {len(df)}. Check payload/columns."
        )
    out = df.copy()
...
def detect_drift(
    ...
    *,
    ...
    common = [
        c for c in ref.columns if c in cur.columns and c not in ignore and c != pred_col
    ]
    ...
    if pred_col in ref.columns and pred_col in cur.columns:
        ...
        pred_gate = (pred_psi >= pred_psi_thresh) or (
            pred_ks_p_thresh > 0 and pred_ks_p <= pred_ks_p_thresh
        )
...
def main():
    ...
    ap.add_argument(
        "--target-col",
        default=None,
        help="optional; passed to retrain script if it needs it",
    )
    ...
    ap.add_argument(
        "--pred-key",
        default=None,
        help="If server returns list of dicts, extract this key (e.g. 'pred_proba').",
    )
    ap.add_argument(
        "--positive-class",
        default=None,
        help="If server returns class labels, treat this label as positive (1).",
    )
    ...
    ap.add_argument(
        "--ignore-cols",
        default="",
        help="Comma-separated columns to skip from FEATURE drift (e.g. ids).",
    )
    ap.add_argument(
        "--any-feature-psi-thresh",
        type=float,
        default=1.0,
        help="Trip if ANY feature PSI ≥ this value.",
    )
    ap.add_argument(
        "--critical-cols",
        default="",
        help="Comma-separated columns with stricter PSI gate.",
    )
    ap.add_argument(
        "--critical-psi-thresh",
        type=float,
        default=0.5,
        help="PSI threshold for CRITICAL columns.",
    )
    ...
    ap.add_argument(
        ...
        type=float,
        default=0.0,
        help="If >0, also trip when prediction KS p ≤ this.",
    )

    # Retrain (Ray) & deploy
    ap.add_argument(
        "--retrain-script", required=True, help="Path to RAY/ray_tune_xgboost.py"
    )
    ap.add_argument(
        "--tracking-uri",
        default="",
        help="MLflow tracking URI (e.g., file:/.../RAY/mlruns)",
    )
    ap.add_argument(
        "--experiment",
        default="xgb_diabetic_readmission_hpo",
        help="Experiment name used by builder",
    )
    ap.add_argument(
        "--build-script", default=None, help="optional: DEPLOY/build_docker_image.py"
    )
    ap.add_argument("--image-tag", default="diabetic-xgb:serve")

    # Ray HPO knobs (forwarded to retrain script)
    ap.add_argument(
        "--hpo-num-samples", type=int, default=15, help="Ray Tune num trials"
    )
    ...
    # Manual override
    ap.add_argument(
        "--force", action="store_true", help="Force retrain regardless of drift gates."
    )
    args = ap.parse_args()
    ...
    ref_scored = score_and_attach(
        ref,
        os.path.join(args.tmp_dir, "ref.csv"),
        args.endpoint,
        args.prediction_col,
        args.pred_key,
        args.positive_class,
    ...
    cur_scored = score_and_attach(
        cur,
        os.path.join(args.tmp_dir, "cur.csv"),
        args.endpoint,
        args.prediction_col,
        args.pred_key,
        args.positive_class,
    ...
    trigger, summary = detect_drift(
        ref_scored,
        cur_scored,
        args.feature_psi_thresh,
        args.ks_p_thresh,
        args.drift_share_thresh,
        args.pred_psi_thresh,
        args.prediction_col,
        ignore=ignore,
    ...
    retrain_cmd = [
        ...
        mlruns_dir,
        "--num-samples",
        str(args.hpo_num_samples),
        "--cpus-per-trial",
        str(args.hpo_cpus),
        "--gpus-per-trial",
        str(args.hpo_gpus),
        "--test-size",
        str(args.hpo_test_size),
        "--ray-dir",
        args.hpo_ray_dir,
        "--seed",
        str(args.hpo_seed),
